...
public sealed class CarSearchTranslationTests
{
    ...
    {
        var query = new CarSearchDto().ToQuery();
    ...
    {
        new CarSearchDto(Location: location).ToQuery().Filters.ShouldBe(expected);
    }
    ...
    public void ToQuery_WithAFractionalRate_WritesItInvariantly()
    {
        ...
        try
        {
            ...
            new CarSearchDto(MaxDailyRate: 12.5m).ToQuery().Filters.ShouldBe("dailyRate<=12.5");
        }
        finally
        {
    ...
    {
        new CarSearchDto(SortBy: sortBy).ToQuery().Sorts.ShouldBe(expected);
    }
    ...
    {
        new CarSearchDto(SortBy: sortBy).ToQuery().Sorts.ShouldBeNull();
    }

    [Fact]
    public void ToQuery_WhenCalled_CarriesFreeTextDatesAndPagingThroughUntouched()
    {
        var request = new CarSearchDto(
            Query: "rav4",
